...
for(i in 1:length(ossl.soillab.level0.names)) {
  ...
  if(iskewness <= -3 | iskewness >= 3) {
    ...
    plot <- ggplot(table.reference.filled, aes(x = label, y = dataset.code_ascii_txt, fill = n)) +
      ...
      scale_x_continuous(breaks = 1:length(pull(percentiles, 2)),
                         labels = round(pull(percentiles, 2), 2),
                         expand = c(0.01, 0)) +
      labs(title = paste0('OSSL level 0: log2(', isoil.property, ')'),
           x = "Percentiles - seq(from=0%, to=100%, by=5%)", y = "", fill = "count") +
      theme_minimal() +
...
for (i in 1:nrow(soillab.level0.desc)) {
  ...
  cat(paste0('\n\n### ', iossl_name))
  cat(paste0('\nDescription: ', idescription, '. It can be alternatively described as `', ianalyte, '`.'))
