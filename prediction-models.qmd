...
To use the OSSL models locally, download pre-trained models and complementary files as provided in [GitHub](https://github.com/soilspectroscopy/ossl-models/blob/main/out/fitted_models_access.csv).

You need to follow a specific directory tree for local runs as described in [ossl_models_directory_tree.csv](https://github.com/soilspectroscopy/ossl-models/blob/main/out/ossl_models_directory_tree.csv).

::: {.callout-tip}
Please note that soil properties names are encoded into a specific string format. Check [fitted_models_performance_v1.2.csv](https://github.com/soilspectroscopy/ossl-models/blob/main/out/fitted_models_performance_v1.2.csv) for the complete list of models as some combinations of spectral regions and soil properties are not available.
:::

You will also need to install R and download the `qs` and `MLR3` ecosystem packages. `qs` is a serialized and compressed file format that is faster than native R `rds`. You need to have [qs package](https://github.com/traversc/qs) version >= 0.25.5 to load files direct from the URLs.

::: {.callout-tip}
For using the pre-trained models with new spectra, the spectra to be predicted must follow these range specifications:  
- VisNIR: 400-2500 nm.  
- NIR Neospectra: 1350-2550 nm.  
- MIR: 600-4000 cm^-1^.  
:::

We provided on GitHub both [examples of datasets](https://github.com/soilspectroscopy/ossl-models/tree/main/sample-data) and a [prediction function](https://github.com/soilspectroscopy/ossl-models/blob/main/R-mlr/OSSL_functions.R) that incorporates all input preparation and output generation.

Please, check the example datasets for formatting your spectra to the minimum required level of the prediction function.

You can provide either `csv`, `asd` (for VisNIR), or opus (`.0`) for MIR scans.

The output table has the prediction value (already back-transformed if log transformation was used) for the soil property of interest, 1-standard-deviation prediction intervals (lower and upper limits), and a flag column for **potential underrepresented** samples.
