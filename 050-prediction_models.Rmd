...
We have received important feedback from people since the first release (Dec. 2021) that said the previous models were doing a great job, but for others not that much. 

Variable model performance may have happened due to the specific soil types not being well represented or due to the spectra not being well aligned with the OSSL.

We conductted a systematic analysis of learning algorithms, global vs local fitting, compression strategies and preprocessing using the OSSL database and the ring trial experiment.

We also tested if including spatial covariates or fusing different spectral regions really help to deliver more reliable predictions. However, this require more time to further verify performance improvements although recent literature have been supporting it.

The following sections describe the steps used to produce the OSSL models, which 
are used in the OSSL Engine and API.

The current available models were fitted using the previous OSSL database version 1.1. For more in-depth insight of the modeling steps please refer to the [ossl-models](https://github.com/soilspectroscopy/ossl-models) GitHub repository.

## OSSL global soil spectroscopy calibrarion models
...
The online calibration service (<https://engine.soilspectroscopy.org>) and the API 
...
ls <- c("plyr", "rgdal", "terra", "mlr", "ranger", "xgboost",
        "Cubist", "fastDummies", "prospectr", "simplerspec", "asdreader")
...
```{r ac-soc1, echo=FALSE, fig.cap="Accuracy plot for `log..oc_usda.calc_wpct/mir_mlr..eml_kssl_na_v1.rds`.", out.width="70%"}
...
```{r pca-sim, echo=FALSE, fig.cap="Similary plot new spectra vs training spectra (PC biplot).", out.width="80%"}
knitr::include_graphics("img/pca_similarity_plot.png")
```
...
```{r ac-soc2, echo=FALSE, fig.cap="Accuracy plot for `log..oc_usda.calc_wpct/visnir_mlr..eml_ossl_na_v1.rds`.", out.width="70%"}
...
```{r, plot-opus, fig.width=6, out.width="90%", fig.cap="Spectral absorbances plot using MIR data."}
...
```{r ac-ph1, echo=FALSE, fig.cap="Accuracy plot for `log..oc_usda.calc_wpct/mir_mlr..eml_ossl_ll_v1.rds`.", out.width="70%"}
knitr::include_graphics("http://s3.us-east-1.wasabisys.com/soilspectroscopy/ossl_models/log..oc_usda.calc_wpct/ap.mir_mlr..eml_ossl_ll_v1.rds.png")
```
...
```{r, plot-ads, fig.width=6, out.width="90%", fig.cap="Spectral reflectance plot using VisNIR data."}
...
```{r ac-phm, echo=FALSE, fig.cap="Accuracy plot for `log..oc_usda.calc_wpct/visnir.mir_mlr..eml_ossl_na_v1.rds` the VisNIR-MIR combination. As compared with other calibration models this seems to be most accurate, although difference with the pure MIR model is not significant.", out.width="70%"}
knitr::include_graphics("http://s3.us-east-1.wasabisys.com/soilspectroscopy/ossl_models/log..oc_usda.calc_wpct/ap.visnir.mir_mlr..eml_ossl_na_v1.rds.png")
```
...
```{r map-bareearth, echo=FALSE, fig.cap="Example of a satellite-based bare-earth spectral product based on @dematte2020bare. The global data is available at 250-m spatial resolution.", out.width="90%"}
knitr::include_graphics("img/bareearth_spectra_250m.jpg")
```
